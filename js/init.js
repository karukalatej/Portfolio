...
function shane_tm_kenburn_slider(){
	...
	"use strict";
		...
		jQuery(function() {
			jQuery('.shane_tm_fixed_image .overlay_slider,.shane_tm_leftside .overlay_slider,.shane_tm_one_page_experience .overlay_slider').vegas({
			timer:false,	
...
function shane_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		jQuery('div.empty_notice').empty(); //To empty previous error/success message.
		//checking for blank fields	
		if(name===''||email===''||message===''){
			jQuery('div.empty_notice').append("<span>Please enter Required Fields!</span>");
			jQuery('div.empty_notice').slideDown(500).delay(2000).slideUp(500);
		}
		else{
			// Returns successful data submission message when the entered information is stored in database.
			jQuery.post("modal/contact.php",{ ajax_name: name, ajax_email: email, ajax_message:message}, function(jsonData) {
				
				isDoing=0;
				var data = JSON.parse(jsonData);
				if(data["success"]===0){
					jQuery('div.empty_notice').append("<span>"+data["text"]+"</span>");
					jQuery('div.empty_notice').slideDown(500).delay(2000).slideUp(500);
				}else{
					jQuery(".contact_form .returnmessage").append(data["text"]);
					
					
					if(jQuery(".contact_form .returnmessage span.contact_error").length){
						jQuery(".contact_form .returnmessage").slideDown(500).delay(2000).slideUp(500);		
					}else{
						jQuery(".contact_form .returnmessage").append("<span class='contact_success'>" +"</span>");
